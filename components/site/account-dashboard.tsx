    ...
    return (
      <div className="page-container py-12">
        <div className="surface-panel p-8 text-center">
          ...
          <p className="mt-3 text-slate-500">Đăng nhập để xem booking, danh sách yêu thích, ticket và toàn bộ dashboard cá nhân.</p>
          <Button asChild className="mt-6 rounded-full bg-sky-600 text-white hover:bg-sky-700">
            <Link href="/login?redirect=/account">Đăng nhập ngay</Link>
          </Button>
        </div>
      </div>
    )
  ...
  return (
    <div className="page-container py-12">
      <div className="mb-10 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <div className="eyebrow">The Horizon account</div>
          <h1 className="mt-3 text-5xl font-black tracking-tight text-slate-950 sm:text-6xl">Xin chào {profile?.full_name || user.email}</h1>
          <p className="mt-4 max-w-3xl text-lg leading-8 text-slate-500">
            Không gian này tổng hợp mọi dữ liệu thật từ booking, thông báo, hành khách đã lưu, danh sách yêu thích và yêu cầu hỗ trợ trong cùng một bảng điều khiển.
          </p>
        </div>
        <div className="flex flex-wrap gap-3">
          <Button asChild className="rounded-full bg-sky-600 px-5 text-white hover:bg-sky-700">
            <Link href="/tours">Đặt thêm tour</Link>
          </Button>
        </div>
      </div>

      {status ? (
        <div className={`mb-6 rounded-[1.6rem] px-4 py-4 text-sm ${["khong", "loi", "error", "failed"].some((keyword) => normalizeSearch(status).includes(keyword)) ? "bg-rose-50 text-rose-700" : "bg-emerald-50 text-emerald-700"}`}>
          {status}
        </div>
      ) : null}
          ...
          <section className="surface-panel p-6">
            <div className="mb-5 flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-sm text-slate-500">Vai trò hiện tại: {primaryRole}</div>
              </div>
            </div>
        ...
        <div className="space-y-6">
          ...
          <section className="surface-panel p-6">
            <div className="mb-4 flex items-center gap-3">
              ...
              <div className="text-2xl font-black tracking-tight text-slate-950">Trạng thái hệ thống</div>
            </div>
            <div className="rounded-[1.5rem] bg-slate-50 p-4 text-sm leading-7 text-slate-600">
              Dashboard này đã nối trực tiếp vào bộ bảng Supabase và các API nội bộ. Khi bạn tạo booking mới, ticket mới hoặc cập nhật profile, giao diện có thể tải lại ngay mà không cần seed thêm dữ liệu.
            </div>
          </section>
        </div>
      </div>
